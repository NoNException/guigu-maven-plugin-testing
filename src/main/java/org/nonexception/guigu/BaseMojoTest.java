...
public abstract class BaseMojoTest {
	...
	/**
	 * <b>Please pass `maven-repo=${you local repository basedir}' to system's properties when running test's</b>
	 * <br>
	 * it while be like
	 * <code> java -Dmaven-repo=${user.home}/.m2/repository</code>
	 *
	 */
	private static String SYSTEM_REPOSITORY_PATH;

	static {
		SYSTEM_REPOSITORY_PATH = System.getProperty("maven-repo");
		// try check  ${user.home}/.m2/repository
		if (Objects.isNull(SYSTEM_REPOSITORY_PATH)) {
	...
	AbstractMojoTestCase preparePluginXml = new AbstractMojoTestCase() {
		MavenProject mockThisPluginMavenProject() {
			...
			project.setArtifactId("example-maven-plugin");
			project.setVersion("any-version");
			project.setName(project.getArtifactId());
			project.setBuild(mockThisPluginBuild());
			Artifact artifact = new DefaultArtifact("org.example", "example-maven-plugin", "any-version", null, "jar", "classifier", null);
	...
	public MojoRule rule = new MojoRule(preparePluginXml) {
		...
		protected void before() throws Throwable {
			...
			if (StringUtils.isEmpty(testMavenProjectDir)) {
